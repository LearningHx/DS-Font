...
from modules.blocks import ResBlock, ConvBlock, w_norm_dispatch, activ_dispatch
...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    if netG == 'resnet_9blocks':
        ...
        net = Attention_Generator(ngf=ngf, use_dropout=use_dropout, n_blocks=6)
    else:
...
class Attention_Generator(nn.Module):
    def __init__(self, ngf=64, use_dropout=False, n_blocks=6):
        ...
        self.content_encoder = FTGAN_Encoder(input_nc=1, ngf=ngf)
        self.decoder = FTGAN_Decoder(use_dropout=use_dropout, n_blocks=n_blocks, ngf=ngf)
        self.attention = cross_attention(ngf*4)
    ...
    def forward(self, inp):
        ...
        content_feature = self.content_encoder(content_image)
        style_features = self.style_encoder(style_images.view(-1, 1, 64, 64))
        attention_fea = self.attention(content_feature,style_features)
        
        feature = torch.cat([content_feature, attention_fea], dim=1)
        outp = self.decoder(feature)
        return outp    
...
class cross_attention(nn.Module):
    def __init__(self, in_planes,max_sample=256 * 256):
        ...
        self.fusion = nn.Conv2d(in_planes, in_planes, (1, 1))
    ...
    def forward(self, content, style,seed=None):
        ...
        Q = self.q(norm_content).view(b,head,int(c//head),-1)
        K = self.k(norm_style).view(b,K,head,int(c//head),-1).permute(0,2,3,1,4).reshape(b,head,c//head,-1)
        #print(b.type(),K.type(),(c//head).type(),head.type())
        V = self.v(style).view(128,6,8,32,-1).permute(0,2,3,1,4).reshape(b,head,c//head,-1)
        logits = torch.einsum("bhcx,bhcy->bhxy",Q,K)
        scores =  self.sm(logits)                                     #global
        out = torch.einsum("bhxy,bhcy->bhcx",scores,V).reshape(b,c,h,w)
        out = self.fusion(out)
